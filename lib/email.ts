...
export async function sendTurnNotificationEmail(to: string, playerName: string, roomCode: string, playerCode: string) {
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      react: TurnNotificationEmail({ playerName, roomCode, gameUrl }),
    })
